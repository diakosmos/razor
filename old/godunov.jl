...
function step!(r::ring; Δt=r.Δt)
    ...
    """ LHS: """
    # right-traveling.
    S_ += r.α⁺[1:end-1] .* Δt .* r₊[1] .* λ₊[1:end-1] .* (λ₊[1:end-1].>0)
    S_ += r.α⁺[1:end-1] .* Δt .* r₋[1] .* λ₋[1:end-1] .* (λ₋[1:end-1].>0)
    F_ += r.α⁺[1:end-1] .* Δt .* r₊[2] .* λ₊[1:end-1] .* (λ₊[1:end-1].>0)
    F_ += r.α⁺[1:end-1] .* Δt .* r₋[2] .* λ₋[1:end-1] .* (λ₋[1:end-1].>0)
    # left-traveling
    S_ += r.α⁻[2:end]   .* Δt .* r₊[1] .* λ₊[2:end] .* (λ₊[2:end].<0)
    S_ += r.α⁻[2:end]   .* Δt .* r₋[1] .* λ₋[2:end] .* (λ₋[2:end].<0)
    F_ += r.α⁻[2:end]   .* Δt .* r₊[2] .* λ₊[2:end] .* (λ₊[2:end].<0)
    F_ += r.α⁻[2:end]   .* Δt .* r₋[2] .* λ₋[2:end] .* (λ₋[2:end].<0)
    """ RHS: """
