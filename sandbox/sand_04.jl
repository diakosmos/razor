...
#using StaticArrays
"""
    Sand 03: "Hyperbolic diffusion", with uniform advection, but with diffusion turned off.
"""
...
#    ∇U_ = diff(U_) ./ ΔX_
    ...
    Δt = 0.2 / max( maximum((U+√q)./ΔX_), -minimum((U-√q)./ΔX_) , s)
    #
    return ring(N,Δt,X_,ΔX_,U, Σ_,S_,J_,F_,zeros(N+1),zeros(N+1),0.0)
end
...
#        α⁻[i] = 0.5 * [1 -1/√q] ⋅ [ΔΣ_[i], ΔJ_[i]]
        α⁺[i] =  [1.0  0.0] ⋅ [ΔΣ_[i], ΔJ_[i]]#
        α⁻[i] =  [0.0  1.0] ⋅ [ΔΣ_[i], ΔJ_[i]]
    end
    r.α⁺ = α⁺
    r.α⁻ = α⁻
end#function
